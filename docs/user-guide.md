...
悬浮键盘可从四条边或四个角直接拖动调整大小，松开后尺寸会自动保存。设置中最多添加 12 个自定义键：选择“输入文字”时直接填写文字；选择“录制按键或组合键”后点击“开始录制”，再在实体键盘按下所需按键，例如 `Ctrl+Shift+S`，界面会自动识别并显示结果，不需要单独填写修饰键。自定义键在右侧每列放置 5 个，超过后自动向右新开一列，不显示滚动条；密码输入目标中全部隐藏。文字内容以明文保存在当前用户的 `config.json` 中，请勿填写密码或密钥。
