...
应用会在可编辑目标获得焦点后自动分类、定位并显示键盘，不需要也不提供手动“捕获目标”步骤。点击目标输入框后即可点击悬浮键盘输入；状态区域会显示目标是否就绪或固定失败提示，且不会暴露 PID、窗口句柄等调试信息。目标切换或身份校验失败时，本次输入会取消，重新聚焦输入框即可。

内置键盘按标准美式 QWERTY 主键区排列，包含完整标点键和左右成对的 Shift/Ctrl/Alt；右侧方向键采用倒 T 排列。Shift、Ctrl、Alt、Win 第一次点击后会保持高亮并参与后续按键，再次点击任一同类键才释放；目标变化、暂停、打开设置或退出也会自动清除。Shift 保持时，数字行和标点键会按当前 Windows 键盘布局输入对应符号。Fn 是本应用的功能层开关，不是可注入的硬件 Fn；Fn 高亮时数字行标签和输入切换为 F1-F12。方向键可直接移动光标。Win 与 Fn 在密码目标中隐藏。
...
悬浮键盘可从四条边或四个角直接拖动调整大小，松开后尺寸会自动保存。设置中的“自定义键标签”和“自定义键输入内容”必须同时填写；保存后该文本键显示在右侧方向键区域上方，点击时按 Unicode 文本发送。两项同时清空即可隐藏该键。自定义键在密码输入目标中不会显示；输入内容以明文保存在当前用户的 `config.json` 中，请勿填写密码或密钥。
