...
- 部分 VS Code 标题栏搜索框需要双击展开后的输入区才显示悬浮按钮，见[调查记录](docs/search-spinner-investigation.md)。Spinner 数值输入框在提供可写 ValuePattern 时受支持；仅能调整数值不构成输入证据，见[兼容性验证](docs/input-compatibility-validation.md)。
