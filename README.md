...
- Some VS Code title-bar search fields currently require a double-click inside the expanded input before the launcher appears; see the [investigation](docs/search-spinner-investigation.md). Numeric Spinner inputs are supported when they expose a writable ValuePattern; range adjustment alone is insufficient. See [compatibility validation](docs/input-compatibility-validation.md).
