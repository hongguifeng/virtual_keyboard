...
- Focus detection now runs in a separate background worker and can recover automatically from blocked accessibility calls. Two application processes are expected. After repeated recovery failures, a tray notification appears; use Resume automatic detection in the tray menu (restart the application if unavailable) and retain its diagnostic logs; see [recovery validation](docs/focus-worker-validation.md).
...
- VS Code title-bar search can be recognized while accessibility focus is delegated to its suggestion list, using an explicit UIA relationship to a writable search input. Numeric Spinner inputs are supported when they expose a writable ValuePattern. Unsupported relationships and range-only controls remain excluded. See [compatibility validation](docs/input-compatibility-validation.md).
- The automated compatibility matrix also covers native numeric, masked, editable combo and rich-text inputs, with read-only controls, selection-only dropdowns, sliders and ordinary lists as negative cases. Run `dotnet test tests/VirtualKeyboard.IntegrationTests -c Release --filter FullyQualifiedName~InputCompatibilityTests` on an interactive Windows desktop.
