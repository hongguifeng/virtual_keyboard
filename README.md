...
`KeyboardLayoutView` 从 JSON 对应的不可变视图模型生成五行按键，行列都使用星号权重，最小按键尺寸为 36×36 DIP。每个按键均不可聚焦、不可进入 Tab 导航，并由控件完整接管鼠标/触摸捕获，不触发 WPF `Button` 的默认 Click 生命周期；按下时显示状态，释放到键外、丢失捕获或取消不会触发动作，同一按下最多触发一次。
