...
启动首帧上限 10 秒，后续通信空闲上限 2 秒，采样无进展上限 5 秒。发送前若采样进展超过 1 秒则返回 FocusIdentityStale；事件变化立即清除最新身份快照，使旧会话无法通过发送前校验。故障/EOF 作废结果代际、清除目标并隐藏自动键盘，终止且确认所启动的唯一子进程退出后，以 1/2/4 秒退避最多重启 3 次。连续健康 30 秒重置失败预算；终止失败或预算耗尽则停止自动恢复，保留日志，需用户重新启动应用。不得用无限创建线程来替代可取消 COM。父进程异常退出关闭 stdin，子进程独立读取线程以进程退出终止卡住的 MTA。正常退出不在 UI 线程等待 UIA 注销。
