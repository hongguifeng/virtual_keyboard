...
MVP 基线使用单进程（REL-030 已将生产焦点采集和分类移至工作进程）。原进程内包含：
...
REL-030：2026-09-10 现场多次调用栈显示原进程 UIA 查询长时间卡滞，独立查询进程正常；据此实现此前预留的独立进程隔离，超出原 MVP 单进程基线。App 用自身可执行文件的内部 `--focus-worker <parent-pid>` 入口启动无窗口工作进程，在单实例/托盘初始化之前分流。工作进程内仍用专用 MTA `FocusObservationService` 完成事件、采集和分类；主进程只消费不可变结果和原生发送前校验，不持有工作进程 AutomationElement。

父子进程通过继承的 stdin/stdout 管道通信，无网络/命名公共端点。每帧上限 8192 字符、RuntimeId 上限 64 个整数，仅传身份/几何/封闭枚举/布尔元数据；RuntimeId 只用于内部校验，不写诊断日志。100 ms 通信帧包含 MTA 完成循环计数、Windows 跨进程 QPC 时间戳、阶段、事件版本和最新结果单槽。传输心跳不能替代采样进展；排队的旧帧也不能刷新输入有效期。

启动首帧上限 10 秒，后续通信空闲上限 2 秒，采样无进展上限 5 秒。发送前若采样进展超过 1 秒则返回 FocusIdentityStale；事件变化立即清除最新身份快照，使旧会话无法通过发送前校验。故障/EOF 作废结果代际、清除目标并隐藏自动键盘，终止且确认所启动的唯一子进程退出后，以 1/2/4 秒退避最多重启 3 次。连续健康 30 秒重置失败预算；终止失败或预算耗尽则停止自动恢复，保留日志。REL-031：托盘提示检测已停止，旧监督任务已完成且子进程终止成功后，用户可从“恢复自动检测”重启有限恢复周期；终止失败则仍须重新启动应用。重新启动沿用原单调版本和代际计数，不允许重复创建活跃工作进程。不得用无限创建线程来替代可取消 COM。父进程异常退出关闭 stdin，子进程独立读取线程以进程退出终止卡住的 MTA。正常退出不在 UI 线程等待 UIA 注销。

父进程为每个新结果重新分配单调焦点版本，通知闭包验证代际/事件变化并在 Dispatcher 应用前再次检查；已关闭工作进程的排队结果不能复活目标。旧事件目标与 ComboBox 分类规则保持原语义。实现及验证见 docs/focus-worker-validation.md。

REL-031：原生焦点回退只读取 FocusHwnd 对应的 AutomationElement 根节点并核验进程及 HasKeyboardFocus，不再调用 FindFirst(Subtree) 遍历后代。现场证明该遍历会进入 MSAA sibling/GetLocation 并长期停滞。无 HWND 编辑器继续使用全局 UIA 焦点和已验证事件目标；两者均不可用时不猜测编辑控件。单个 UIA 属性查询仍由进程超时边界保护。托盘每秒检查监督状态，耗尽时一次气泡提示并持续显示停止状态的 tooltip；手动恢复记录 FocusWorkerRearmed，超时日志携带工作进程 PID。
...
REL-029 补充兼容回归：TestHost 支持 `--compatibility-case <scenario>`，提供 numeric、numeric-readonly、masked、combo-edit、combo-select、richtext、richtext-readonly、slider、list 九个合成控件场景。集成矩阵复用一个窗口，通过有界 stdin/ready 握手切换场景，再启动生产隔离检测进程验证真实 UIA 判定。测试夹具使用与既有 NoActivate 集成测试相同的输入队列附加方法建立前台前提，并立即解除；此能力不进入产品。日志原因码同步映射 SearchInputRelationship，避免成功的搜索关系被误记为 ElementInvalid。
...
REL-025 的视觉层仅位于 WPF View：MainWindow 使用透明宿主内的 14 DIP 圆角深色 Chrome，键帽模板使用石墨色背景、7 DIP 圆角、分层边框及 hover/pressed trigger；锁定修饰键继续由 `KeyboardControllerState` 驱动，但强调色统一为蓝色 `#2878F0` 和浅蓝边框。设置窗口保留可激活的系统窗口语义，内容改为浅色卡片并统一 TextBox、ComboBox、CheckBox、Button、GroupBox 与列表样式。全部行列仍使用原 Star 权重和最小尺寸，样式不接触输入事件、TargetSession、Overlay 或配置透明度。README 截图使用实际运行窗口重新采集。
...
REL-022 在 schema v1 快照末尾新增 `uiLanguage`，封闭值为 `English` 和 `SimplifiedChinese`。该字段为兼容性可选字段：旧配置缺失时按 `English` 迁移，非法枚举仍由统一验证器拒绝。App 层 `AppStrings` 集中管理设置窗口、主窗口状态和托盘菜单文案；设置窗口语言下拉框即时刷新文案，保存后 MainWindow 与托盘从 Repository 当前快照重新取值，默认语言为英语。键盘标准键帽及用户自定义内容不参与翻译。
