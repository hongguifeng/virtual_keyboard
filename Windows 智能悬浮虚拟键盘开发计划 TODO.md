...
### REL-029 观测阶段：UIA 事件目标与查询目标对照

- [x] TestHost --focus-probe 独立只读模式；有界队列/时间/行数，新文件写入，数字错误与隐私字段白名单测试。
- [x] 第一轮 90 秒现场采集：403 行、42 对事件/查询、23 对身份不同；无丢事件或读取/写入错误，工作线程正常停止。尚无 VS Code 故障样本，不据此放宽产品判定或宣称修复。
...
### REL-031 焦点回退遍历卡滞与耗尽后恢复

- [x] 根据现场栈删除 Native Focus HWND 下 FindFirst(Subtree)；回退仅核验根元素的进程与焦点，保留全局 UIA 和事件目标规则。
- [x] 托盘显示检测停止状态并提供有界主动恢复；仅旧任务完成且子进程确认终止时可重启，保持代际与单调版本，防重复启动。
- [x] 新增 FocusWorkerRearmed 诊断，超时事件携带工作进程 PID；无输入内容。
- [x] 完整 Release 构建零警告/错误，Core 245 + Windows 251 + Integration 66 = 562 项通过；TestHost --selftest 退出 0；45 秒真实工作进程观测进度持续推进。详见 docs/focus-worker-validation.md。
...
- [x] 按用户要求新增独立的 `launcherCustomKeys` 配置；schema v1 缺失时为空，与键盘内自定义键分别最多 12 项，复用动作白名单、长度限制和配置恢复。设置保存、自启同步/回滚、启停、尺寸保存均保留两组。
- [x] 设置编辑器增加“显示位置：键盘内 / 悬浮按钮”，切换时提交编辑并停止录制，支持两组独立增删、中英切换、重开设置及录制组合键；录制按钮宽度随文案自适应。
- [x] 保留最左侧展开按钮，自定义按钮按配置顺序向右并排；全部 40×40 DIP、间距 4 DIP，DPI 和小工作区中整行等比约束。自定义动作直接执行，不展开键盘；密码目标只显示展开按钮。
- [x] 显示会话绑定与呈现代际拒绝旧事件；目标变化、隐藏、设置、暂停和退出取消手势/重复及捕获。发送复用有界队列与独立目标复核；失败提供有界非激活提示，不回显自定义内容。
- [x] `scripts/build.ps1 -SkipPackage`：Release 0 警告/错误，Core 277 + Windows 251 + Integration 100 = 628 项通过，无跳过；TestHost WPF/WinForms 自检退出 0。原生鼠标→文字输入与展开保持前台/焦点的集成断言、整行 100–200% DPI 及中英文 WPF 渲染检查通过，详见 `docs/launcher-custom-buttons-validation.md`。
...
- [x] 后续 Spinner 修复已实现类型与可写值证据支持，补齐安全门、协议与负例测试，并由生产 worker 和用户输入复验真实数值框。
- [x] 后续搜索修复已用有界探针证实输入宿主与列表关系，加入双身份和发送校验；不直接放行普通 ListItem/Other，不恢复子树遍历。
- [ ] 退出检查：独立主审查通过。调查记录中的两个问题已在后续修复中实现，并完成现场输入确认；本节保留原始调查证据。

### REL-029 Spinner 修复（2026-09-11）

- [x] 保留 Spinner 类型，以可写 ValuePattern 作为输入证据；只读、失焦、禁用、离屏、无身份和无可写值证据仍拒绝，未放宽 Other/Slider/ListItem。
...
- [x] 整合版实际悬浮显示与输入复验：用户确认 WorkEnglish Coach 数值框正常输入，见兼容性验证记录。
- [ ] 退出检查：独立主审查。
...
- [x] 整合版实际显示/输入：现场单击搜索入口即显示按钮，用户确认正常输入；补充九场景原生控件矩阵完成。
- [ ] 退出检查：独立主审查。

### REL-029 输入兼容矩阵与诊断补充（2026-09-11）

- [x] 新增 TestHost 原生数值、掩码、可编辑下拉、富文本及对应只读/仅选项/滑块/列表反例，共九场景，通过真实生产 worker 集成验证。窗口管理器导致首轮前台前提失败的问题已在测试夹具修正，未削弱产品规则或断言。
- [x] 修正搜索关系原因码在应用日志的映射；回归先失败后通过。
- [x] 完整 Release 构建 0 警告/错误，296 + 280 + 102 = 678 项通过，0 跳过（一个矩阵测试含九个场景）；TestHost WPF 31、WinForms 36 项自检通过。同步设计、双语 README 与 `docs/input-compatibility-validation.md`。
- [x] 整合版现场显示/输入记录：`1.0.13-local` 已运行，用户确认两个问题输入框均正常输入。
- [ ] 退出检查：独立主审查。
